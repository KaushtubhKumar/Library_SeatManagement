/**
 * scripts/seed-floor1.ts
 *
 * Seeds floor 1 with real Zone/Seat rows matching the redesigned
 * layout in lib/floorPlanConfig.ts — desk blocks (A-E) plus, new in
 * this version, real bookable seats for both discussion rooms (6
 * each, arranged in a circle) instead of those being decoration-only.
 *
 * Deletes floor 1's existing zones/seats first — safe to re-run.
 * Usage: npx tsx scripts/seed-floor1.ts
 */
  ...
  if (block.orientation === "h") {
    ...
    for (let r = 0; r < block.rows; r++) {
      ...
      for (let k = 0; k < perSide; k++) {
        ...
        const push = (y: number, suffix: string) =>
          seats.push({
            seatCode: `F1-${block.id}-T${r + 1}-S${String(k + 1).padStart(2, "0")}${suffix}`,
            posX: x / CANVAS_WIDTH,
            posY: y / CANVAS_HEIGHT,
            xPx: x,
            yPx: y,
          });
        if (singleSided) push(tableY - block.rowOffset, "");
        else {
          push(tableY - block.rowOffset, "A");
          push(tableY + block.rowOffset, "B");
        }
      }
    }
  } else {
    ...
    for (let r = 0; r < block.rows; r++) {
      ...
      for (let k = 0; k < perSide; k++) {
        ...
        seats.push({ seatCode: `F1-${block.id}-T${r + 1}-S${String(k + 1).padStart(2, "0")}A`, posX: (tableX - block.rowOffset) / CANVAS_WIDTH, posY: y / CANVAS_HEIGHT, xPx: tableX - block.rowOffset, yPx: y });
        seats.push({ seatCode: `F1-${block.id}-T${r + 1}-S${String(k + 1).padStart(2, "0")}B`, posX: (tableX + block.rowOffset) / CANVAS_WIDTH, posY: y / CANVAS_HEIGHT, xPx: tableX + block.rowOffset, yPx: y });
      }
    }
  }
  return seats;
...
/** Circular arrangement — capacity seats evenly spaced around the
 * room's center, starting from the top and going clockwise. */
function computeCircularSeats(room: DiscussionRoom): SeatCoord[] {
  ...
  for (let i = 0; i < room.capacity; i++) {
    ...
    seats.push({
      seatCode: `F1-${room.id}-S${String(i + 1).padStart(2, "0")}`,
      posX: x / CANVAS_WIDTH,
      posY: y / CANVAS_HEIGHT,
      xPx: x,
      yPx: y,
...
function checkForOverlaps(id: string, radius: number, coords: SeatCoord[]) {
  for (let i = 0; i < coords.length; i++) {
    for (let j = i + 1; j < coords.length; j++) {
      const dx = coords[i].xPx - coords[j].xPx;
      const dy = coords[i].yPx - coords[j].yPx;
      const edgeGap = Math.sqrt(dx * dx + dy * dy) - radius * 2;
      if (edgeGap < MIN_SAFE_GAP_PX) {
        console.warn(`⚠ ${id}: ${coords[i].seatCode} / ${coords[j].seatCode} only ${edgeGap.toFixed(1)}px apart`);
      }
    }
  }
}
...
async function main() {
  ...
  try {
    ...
    for (const block of FLOOR_1_SEAT_BLOCKS) {
      ...
      await client.query(
        `INSERT INTO "Zone" (id, "floorId", name, "zoneType", "declaredNoiseLevel") VALUES ($1, $2, $3, $4, $5)`,
        [zoneId, floorId, block.name, block.zoneType, noiseLevel]
      );
    ...
    // Discussion rooms — now real bookable GROUP_TABLE seats, not decoration
    for (const room of FLOOR_1_DISCUSSION_ROOMS) {
      const zoneId = randomUUID();
      await client.query(
        `INSERT INTO "Zone" (id, "floorId", name, "zoneType", "declaredNoiseLevel") VALUES ($1, $2, $3, 'DISCUSSION', 4)`,
        [zoneId, floorId, room.name]
      );
...
main().catch((err) => {
  console.error("Floor 1 layout seed failed:", err);
  process.exit(1);
});